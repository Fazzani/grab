...
parser.add_argument('--path', '-p', help='Fill from git history',
                    action='store', dest='file_path')
parser.add_argument('--stats', '-s', help='Calculate statistics',
                    action='store_true', default=False, dest='is_stats')
...
def stats() -> None:
    ...
    df["missed_count"] = df[df.missed == True].missed.groupby(
        df.id).transform("count")
